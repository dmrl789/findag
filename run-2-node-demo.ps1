# FinDAG 2-Node Demo: Full Automation
# This script automates a 2-node, 2-bot demo with logging and monitoring

$ErrorActionPreference = 'Stop'

Write-Host "🚀 FinDAG 2-Node Demo (Automated)" -ForegroundColor Green
...
# 4. Start 2 nodes in background, each with unique ports and data dir
$nodes = @(
    @{ name = "Node1"; http = 3001; udp = 9001; data = "nodes/node1"; peers = "127.0.0.1:9002"; log = "logs/node1.log" },
    @{ name = "Node2"; http = 3002; udp = 9002; data = "nodes/node2"; peers = "127.0.0.1:9001"; log = "logs/node2.log" }
)

Write-Host "🌐 Starting 2 nodes in background..." -ForegroundColor Yellow
foreach ($n in $nodes) {
...
# 7. Start 2 bots in background, each targeting a different node
$bots = @(
    @{ name = "Bot1"; url = "http://127.0.0.1:3001"; log = "logs/bot1.log" },
    @{ name = "Bot2"; url = "http://127.0.0.1:3002"; log = "logs/bot2.log" }
)
Write-Host "🤖 Starting 2 transaction bots in background..." -ForegroundColor Yellow
foreach ($b in $bots) {
...
Write-Host "🎉 2-Node Demo is Running!" -ForegroundColor Green
Write-Host "=================================" -ForegroundColor Green
Write-Host ""
Write-Host "📊 Node logs: logs/node1.log, logs/node2.log" -ForegroundColor White
Write-Host "🤖 Bot logs: logs/bot1.log, logs/bot2.log" -ForegroundColor White
Write-Host ""
