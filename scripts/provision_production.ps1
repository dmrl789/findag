...
.EXAMPLE
    .\provision_production.ps1 -Environment production -Region us-east-1 -NodeCount 3
...
function Set-SecurityHardening {
    if ($SkipSecurity) {
        Write-Log "Skipping security hardening as requested" -Level "WARN"
        return
    }
    
    Write-Log "Configuring security hardening..."
...
"@ | kubectl apply -f -
    ...
    Write-Log "Security hardening configured" -Level "SUCCESS"
}
...
function Test-HealthChecks {
    ...
    if ($serviceIP) {
        $healthResponse = Invoke-RestMethod -Uri "http://$serviceIP:8080/health" -Method Get
